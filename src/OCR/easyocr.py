...
def video_to_text(video_file_path: str, capture_every_n_seconds: int, frame_diff_threshold: int, device=None, debug=False):
    if device and reader.device != device:
        print(
            f"Easy OCR automatically selects the most efficient available device. Current selection: {reader.device}.")
    ...
    while True:
        ...
        result = reader.readtext(
            current_frame, paragraph=True, y_ths=0.5, mag_ratio=1.5, detail=0)
        text = "\n".join(result)
